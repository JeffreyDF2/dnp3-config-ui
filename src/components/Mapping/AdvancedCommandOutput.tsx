...
export default function AdvancedCommandOutput({
  index,
  mapping,
  onChange,
}: Props) {
  ...
  useEffect(() => {
    ...
    if (mapping.index < 0 || mapping.index > 1)
      newErrors.index = "Index must be 0 or 1";
      ...
      <div className="grid grid-cols-2 gap-4 border p-4 rounded-md shadow-sm">
        ...
        <div className="space-y-1.5">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="Trip Breaker">Trip Breaker</SelectItem>
              <SelectItem value="Close Breaker">Close Breaker</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="space-y-1.5">
          <Tooltip>
            <TooltipTrigger asChild>
              <Label htmlFor={`cmd-group-${index}`}>Group</Label>
            </TooltipTrigger>
        ...
        <div className="space-y-1.5">
          <Tooltip>
            <TooltipTrigger asChild>
              <Label htmlFor={`cmd-variation-${index}`}>Variation</Label>
            </TooltipTrigger>
            <TooltipContent>{mappingEntryTooltips.variation}</TooltipContent>
          </Tooltip>
          <Input
            id={`cmd-variation-${index}`}
            type="number"
            min={0}
        ...
        <div className="space-y-1.5">
          <Tooltip>
            <TooltipTrigger asChild>
              <Label htmlFor={`cmd-index-${index}`}>Index</Label>
            </TooltipTrigger>
...
// This component provides an advanced interface for configuring command output mappings.
